...
- **`StatusCode` value convention (resolved).** The collector writes the contrib values
  `"Ok"` / `"Error"`; OTLP `"Unset"` is collapsed to `"Ok"` in the parser, so `"Unset"` is
  never emitted (flag for a parser change if Pod 3 needs that distinction).
