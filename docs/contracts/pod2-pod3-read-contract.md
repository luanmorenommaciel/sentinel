...
| Source of truth | [`services/collector-rust/infra/clickhouse/ddl/`](../../services/collector-rust/infra/clickhouse/ddl/) (executable schema) |
...
> **Monorepo-integration note (2026-06-16).** This contract documents the **Rust**
> collector's ClickHouse schema (`services/collector-rust/`). The monorepo also
> ships a **Go** collector (`services/collector-go/`) whose DDL diverges from this
> one (e.g. `otel_spans` vs `otel_traces`, snake_case vs PascalCase, raw
> `*_unix_nano` + ingestion-time partitioning vs `DateTime64(9,'UTC')` +
> signal-time, `Nullable` IDs vs the `''` sentinel, no `otel_metrics_1m` rollup).
> See [`docs/clickhouse-schema-divergence.md`](../clickhouse-schema-divergence.md).
> **Selecting the canonical read schema (and whether Go must conform) is a pending
> Crew B decision** — see the sync proposal in
> [`docs/proposals/canonical-read-schema.md`](../proposals/canonical-read-schema.md).
> Until that lands, Pod 3 should build against **this** (Rust) schema, which is the
> only one with a decision record (ADR-0005/0006) and a live + CI round-trip gate.
...
- [`services/collector-rust/infra/clickhouse/ddl/`](../../services/collector-rust/infra/clickhouse/ddl/) — executable schema (source of truth)
